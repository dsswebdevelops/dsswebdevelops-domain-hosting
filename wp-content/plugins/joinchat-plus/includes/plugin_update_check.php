...
if ( ! class_exists( 'PluginUpdateChecker_2_0' ) ) :
	...
	class PluginUpdateChecker_2_0 {
		...
		public $debugMode = true;// false; //Set to TRUE to enable error reporting. Errors are raised using trigger_error()
		...
		public function requestInfo( $queryArgs = array(), $noPlugins = false ) {
			...
			} elseif ( ! is_wp_error( $result ) && isset( $result['response'] ) && isset( $result['response']['code'] ) && ( $result['response']['code'] == 401 ) ) {
				$pluginName = $this->getPluginName();
				update_option( "{$pluginName}-invalid-notice", 'show', 'yes' );
			} elseif ( ! is_wp_error( $result ) && ! $noPlugins && isset( $result['response'] ) && isset( $result['response']['code'] ) && ( ( $result['response']['code'] == 400 ) || ( $result['response']['code'] == 431 ) ) ) {
